...
function buildPage(root){
  ...
  root.append(S('s-catch',[el('div',{class:'wrap'},[
    ...
    el('p',{class:'sub'},['We assumed the cleverest scammer would simply beat the field. It did — and made the rest of the system better while doing it.'])]),
    el('div',{class:'wrap'},[fig(figCoupling(),
      'Left: spread the payments around and the recipient monitor stays quiet, but the declared purpose matches none of those accounts. Right: use only accounts that fit the declared purpose and the purpose check goes quiet — but the payments bunch into a handful of accounts, and the recipient monitor fires.',
      'Two panels comparing dispersed routing, where the purpose check fires, against purpose-matched routing, where the recipient check fires instead')]),
    el('div',{class:'wrap'},[take('why this matters','A scammer can look consistent <b>or</b> stay spread out — not both. So the field is worth more than what it catches by itself: it also forces the attacker into a shape the checks you already run can see. We found no published statement of this trade-off.','good')])]));
