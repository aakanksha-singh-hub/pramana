...
function renderDecide(root){
  ...
  if(c){
    ...
    rows.push(['Holds across repeat runs',c.significant?'positive in all '+c.n_seeds+' independent repeats':'no — interval includes zero']);
    rows.push(['Smallest menu we tested that worked','6 categories · 3 carried no measurable value']);
...
function renderLanding(root){
  ...
  root.append(el('section',{class:'blk land'},[el('div',{class:'wrap'},[
    ...
    el('p',{class:'lede'},['When someone is tricked into paying a scammer, they approve the payment themselves. So every check the bank runs says yes — correctly. The missing context is simple: what the payer thought they were paying for.']),
    el('p',{class:'lede',style:'margin-top:14px'},['Pramana adds that context to the same fraud model — the one a bank already runs — and shows you what changes.']),
...
function renderFound(root){
  ...
  root.append(S('s-catch',[el('div',{class:'wrap'},[
    ...
    el('p',{class:'sub'},['We expected the cleverest scammer to simply beat the new context. In our simulation it turned out they have to choose.'])]),
      ...
      (()=>{const t=el('table',{class:'cmp'});
        ...
        t.append(el('tbody',{},[
          ...
          el('tr',{class:'verdict'},[el('td',{},['So what happens?']),
            el('td',{},[el('b',{style:'color:var(--neg)'},['The purpose–account check catches them'])]),
            el('td',{},[el('b',{style:'color:var(--neg)'},['The account check catches them'])])])]));
        return t;})()])]),
    el('div',{class:'wrap'},[take('why this matters','In our simulated attack they can look consistent <b>or</b> stay spread out — not both. The declared context either catches the mismatch, or pushes the attack into a pattern the account checks already see. We found no published statement of this trade-off.','good')])]));
