...
function buildPage(root){
  /* 1 — HERO IS THE PRODUCT */
  root.append(S('s-top',[el('div',{class:'wrap'},[
    el('h1',{class:'hero'},['See what one missing field does to a fraud decision.']),
    el('p',{class:'lede'},['Below is a real fraud model, running twice on the same payment. The only difference is that the second one is told what the payer says the money is for. Change anything and watch the decision move.'])]),
  ...
  root.append(S('s-feel',[el('div',{class:'wrap'},[
    ...
    el('p',{class:'sub'},['Scam victims authorise their own transfers, so every existing control correctly says yes. Here are two real accounts from our simulation — both take money from dozens of unrelated people and pass it straight on. One collects scam money. These are the only fields a fraud system has.'])]),
